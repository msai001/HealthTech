...
func main() {
	...
	log.Printf("HealthOS v10.0 | Full Recovery | Port: %s", port)
...
func sendOTPEmail(toEmail string, code string) {
	...
	if from == "" || pass == "" {
		log.Println("Ошибка: Переменные почты не настроены на Render")
		return
	}

	subject := "Subject: HealthOS: Твой код доступа\n"
	mime := "MIME-version: 1.0;\nContent-Type: text/html; charset=\"UTF-8\";\n\n"
	body := fmt.Sprintf(`
		<div style="font-family:sans-serif; text-align:center; padding:20px; border:1px solid #e2e8f0; border-radius:16px;">
			<h2 style="color:#2563eb;">Код подтверждения</h2>
			<div style="font-size:36px; font-weight:bold; color:#0f172a; margin:20px 0;">%s</div>
			<p style="color:#64748b;">Введите этот код для завершения входа.</p>
		</div>`, code)
	...
	fmt.Fprint(w, `
		<body style="font-family:sans-serif; background:#0f172a; display:flex; justify-content:center; align-items:center; height:100vh; margin:0; color:white;">
			<div style="background:#1e293b; padding:40px; border-radius:24px; text-align:center; width:350px; border:1px solid #334155;">
				<h2 style="color:#38bdf8;">🛡️ Подтверждение</h2>
...
func handleVerifyOTP(w http.ResponseWriter, r *http.Request) {
	...
	err = db.QueryRow(query).Scan(&dbOtp, &name, &userEmail)
	if err != nil {
	...
	log.Printf("[AUTH] Ввод: %s | База: %s | Юзер: %s", input, dbOtp, userEmail)
	...
	<body>
		...
		<div class="main">
			<div id="doc-panel" class="card" style="display:none;">
				<h3>Панель врача</h3>
				<input id="p-mail" placeholder="Email пациента"> 
				<input id="p-diag" placeholder="Диагноз">
			...
			function refresh() {
				fetch('/api/data').then(r => r.json()).then(data => {
					document.getElementById('list').innerHTML = data.map(d => 
						'<tr><td>'+d.date.split('T')[0]+'</td><td><b>'+d.name+'</b></td><td style="text-align:right">'+(d.diag || 'Обработка')+'</td></tr>'
					).join('');
				});
			}
			function save() {
