...
const sharedStyles = `
	<style>
		...
		.otp-input { font-size: 28px; letter-spacing: 8px; text-align: center; width: 80%; margin-top: 20px; border: 1px solid #e2e8f0; border-radius: 12px; padding: 10px; }
...
func sendEmailOTP(toEmail, code string) {
	...
	msg := fmt.Sprintf("To: %s\nSubject: HealthTech Code\n\nYour security code: %s", toEmail, code)
...
func main() {
	...
	http.HandleFunc("/otp-check", func(w http.ResponseWriter, r *http.Request) {
		...
		// ИСПРАВЛЕННАЯ СТРОКА (добавлено , _)
	...
	http.HandleFunc("/dashboard", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, "<html><head><meta charset=\"UTF-8\">%s</head><body><div class=\"card\"><h3>🌿 Журнал: %s</h3><p>Вы успешно вошли в систему!</p><br><a href=\"/logout\" style=\"color:red;\">Выйти</a></div></body></html>", sharedStyles, email)
	})
