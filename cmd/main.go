...
const sharedStyles = `
	<style>
		...
		.btn { cursor: pointer; border: none; border-radius: 12px; font-weight: 700; padding: 14px; background: #10b981; color: white; width: 100%; display: block; text-decoration: none; margin-top: 10px; }
...
func main() {
	...
	http.HandleFunc("/callback", func(w http.ResponseWriter, r *http.Request) {
		...
		http.SetCookie(w, &http.Cookie{Name: "user_email", Value: userInfo.Email, Path: "/", Expires: time.Now().Add(60 * time.Minute)})
	...
	http.HandleFunc("/otp-verify", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, "<html><head><meta charset='UTF-8'>%s</head><body><div class='card'><h2>Введите код</h2><form action='/otp-check' method='POST'><input type='text' name='code' class='otp-input' required autofocus><button type='submit' class='btn'>Подтвердить</button></form></div></body></html>", sharedStyles)
	})
	...
	http.HandleFunc("/otp-check", func(w http.ResponseWriter, r *http.Request) {
		...
		if valid {
			// Ставим куку успешной сессии на 30 минут
			http.SetCookie(w, &http.Cookie{Name: "session_valid", Value: "true", Path: "/", Expires: time.Now().Add(30 * time.Minute)})
	...
	http.HandleFunc("/dashboard", func(w http.ResponseWriter, r *http.Request) {
		...
		if emailCookie == nil || sessionCookie == nil {
			http.Redirect(w, r, "/", 302)
			return
		}
	...
	http.HandleFunc("/add-appointment", func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == "POST" && cookie != nil && session != nil {
			db.Exec("INSERT INTO appointments (user_email, doctor_name, appointment_date, patient_name, totp_secret) VALUES ($1, $2, $3, $4, '')",
				cookie.Value, r.FormValue("doc"), r.FormValue("date"), "Пациент")
		}
		// Теперь перенаправляем сразу в дашборд!
		http.Redirect(w, r, "/dashboard", 302)
	})
